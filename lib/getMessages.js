...
const UserNameValidate = Joi.string().default('').description('Name of user who posts the message. This field is required when TimeStamp exists, otherwise can be empty.');
const TimeStampValidate = Joi.string().default('').description('The timestamp when the message is posted. Will return all msgs posted by the user if this field is missing. Must be used with UserName together');
const getMessagesValidate = Joi.object().keys({
    ...
    StartKey: Joi
        ...
        .description('Query will start from this key instead of the beginning of DB Table. Empty will start from beginning. Ex: {"UserName": A_USER_NAME_STRING, "TimeStamp": A_TIME_STAMP_STRING}')
});

// Generate params depends on parameters in the request
const queryMessagesParams = (query) => {
    let expAttrNames = {},
        expAttrValues = {},
        filterExp = [];
...
*/
const getMessages = (req, resp) => {
    let query = req.query,
        dbParams = queryMessagesParams(query);

    dbParams.func.call(dynamodb, dbParams.params, (err, data) => {
        if (err) {
            console.log(err);
