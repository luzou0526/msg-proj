...
const deleteMessageValidate = Joi.object().keys({
    UserName: Joi
        .string()
        .default('')
        .description('Name of user who posts the message. Non-exist name will return success but has no effect.'),
    TimeStamp: Joi
        .string()
        .default('')
        .description('The timestamp when the message is posted. Non-exist time will return success but has no effect.')
}).requiredKeys(
    'UserName', 'TimeStamp'
);
